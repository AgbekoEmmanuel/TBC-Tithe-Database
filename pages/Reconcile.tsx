  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
           <h2 className="text-3xl font-bold text-slate-800">Cash Reconciliation</h2>
           <p className="text-slate-500 mt-1">Verify physical cash against system entries.</p>
        </div>
        <div className="bg-white px-6 py-3 rounded-xl shadow-sm border border-slate-100">
           <span className="text-slate-400 text-xs font-bold uppercase">System Expected Cash</span>
           <div className="text-2xl font-black text-slate-800">GH₵{systemCash.toLocaleString()}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-[2rem] p-8 shadow-sm">
          <div className="flex items-center mb-6 text-slate-800 font-bold text-lg">
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-4">
            {[200, 100, 50, 20, 10, 5, 2, 1].map((denom) => (
              <div key={denom} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                <div className="font-bold text-slate-600 w-16">GH₵{denom}</div>
                <div className="flex items-center">
                  <span className="text-xs text-slate-400 mr-2">x</span>
                  <input
                    type="number"
                    min="0"
                    className="w-24 text-right font-mono font-bold text-lg bg-white border border-slate-200 rounded-lg px-2 py-1 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                    value={counts[denom as keyof typeof counts] || ''}
                    onChange={(e) => handleCountChange(denom, e.target.value)}
                  />
                </div>
                <div className="w-24 text-right text-sm text-slate-400 font-medium">
                  = {(denom * (counts[denom as keyof typeof counts] || 0)).toLocaleString()}
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-6">
          {/* Status Card */}
          <div className={`rounded-[2rem] p-8 shadow-sm text-white transition-colors ${
